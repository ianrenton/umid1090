...
//
// MODIFIED BY IAN RENTON FOR UMID 1090
//
...
function getAircraftData(icao, server) {
        var defer;
        ...
        if (icao in _aircraft_cache) {
                ...
                request_from_db(icao, 1, defer, server);
        }

        return defer;
}

function request_from_db(icao, level, defer, server) {
        var bkey = icao.substring(0, level);
        var dkey = icao.substring(level);
        var req = db_ajax(bkey, server);

        req.done(function(data) {
                var subkey;

                if (dkey in data) {
                        getIcaoAircraftTypeData(data[dkey], defer, server);
                        return;
                }
                ...
                if ("children" in data) {
                        ...
                        if (data.children.indexOf(subkey) != -1) {
                                request_from_db(icao, level+1, defer, server);
                                return;
                        }
                }
                defer.reject();
...
function getIcaoAircraftTypeData(aircraftData, defer, server) {
    if (_aircraft_type_cache === null) {
        $.getJSON(server + "db/aircraft_types/icao_aircraft_types.json")
...
function db_ajax(bkey, server) {
        ...
        if (_request_count < MAX_REQUESTS) {
                ...
                ++_request_count;
                defer = _request_cache[bkey] = $.ajax({ url: server + 'db/' + bkey + '.json',
                                                        cache: true,
                                                        timeout: 5000,
                                                        dataType : 'json' });
                defer.always(function() {db_ajax_request_complete(server); });
        } else {
                // put it in the queue
...
function db_ajax_request_complete(server) {
        var req;
        var ajaxreq;
